...
public class World {
        ...
        try {
            ...
            for(int xx = 0; xx < map.getWidth();xx++){
                for(int yy = 0; yy < map.getHeight();yy++){
                    ...
                    else if(pixelAtual == 0xFF6BAAFF){
                        //Question Mark
                        Game.entities.add(new QuestionMark(xx*TILE_SIZE,yy*TILE_SIZE,16,16,Entity.QUESTIONMARK_EN));
                    }
                    else if(pixelAtual == 0xFFffff00){
                        //DoorTile
                        tiles[xx + (yy*WIDTH)] = new DoorTile(xx*TILE_SIZE,yy*TILE_SIZE,Tile.TILE_DOOR);

                    }
                    else if(pixelAtual == 0xFFFF7200){
                        //Panel
                        tiles[xx + (yy*WIDTH)] = new WallTile(xx*TILE_SIZE,yy*TILE_SIZE,Tile.TILE_PANEL);
                    }
                }
            }
        } catch (IOException e) {
    ...
    public static boolean doorOpen(int xNext, int yNext){
        int x1 = xNext / TILE_SIZE;
        int y1 = yNext / TILE_SIZE;
        int x2 = (xNext + TILE_SIZE - 1) / TILE_SIZE;
        int y2 = yNext / TILE_SIZE;
        ...
        return !(tiles[x1 + (y1 * World.WIDTH)] instanceof DoorTile ||
                tiles[x2 + (y2 * World.WIDTH)] instanceof DoorTile ||
                tiles[x3 + (y3 * World.WIDTH)] instanceof DoorTile ||
                tiles[x4 + (y4 * World.WIDTH)] instanceof DoorTile);
    }
